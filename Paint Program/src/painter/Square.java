...
public class Square extends General implements Cloneable, Drawer, ShapeModfier {

    private Point ptemp;
    private boolean first = true;
    Square sq = null;

    public Square(Point p1, Point p2, Color color, boolean fill, int undoRedoType) {
    ...
    public void draw(Graphics m) {
        ...
        m.setColor(color);
        if (!fill) {
            m.drawRect(x, y, avg, avg);
        } else {
            m.fillRect(x, y, avg, avg);
        }
    }
    ...
    public void adjust(Graphics m, Point pd, Point pf, int type) {
        switch (type) {
            case (1):
                if (first) {
                    Square sq2 = new Square(p1, p2, color, fill, undoRedoType);
                    Painter.type2.add(sq2);

                    sq = (Square) Painter.main.get(Painter.index);
                    sq.setP2(pd);
                    sq.setUndoRedoType(2);
                    Painter.main.remove(Painter.index);
                    Painter.main.add(sq);
                    first = false;
                } else {
                    sq = (Square) Painter.main.get(Painter.main.size() - 1);
                    sq.setP2(pd);
                    sq.setUndoRedoType(2);
                }
                break;
    ...
    public void copy(Point p, General selected) {
        ...
        Square stemp = (Square) selected;
        Square s = null;
        try {
            s = (Square) stemp.clone();
    ...
    protected Object clone() throws CloneNotSupportedException {
        ...
        sq.p1 = (Point) p1.clone();
        sq.p2 = (Point) p2.clone();
        return sq; //To change body of generated methods, choose Tools | Templates.
    }
